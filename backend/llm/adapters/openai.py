"""OpenAI-compatible adapter for the unified LLM layer.

Also powers OpenRouter, DeepSeek, and Kimi (Moonshot) because they expose an
...
class OpenAIAdapter(LLMProvider):
    """OpenAI-compatible adapter via langchain-openai."""

    name = "openai"
    capabilities = ProviderCapability.TEXT | ProviderCapability.JSON | ProviderCapability.TOOLS | ProviderCapability.STREAM

    def __init__(self, api_key: str, model: str, base_url: str | None = None, **extra: Any) -> None:
    ...
    def _get_chat_model(self, temperature: float = 0.0, max_tokens: int | None = None) -> ChatOpenAI:
        kwargs: dict[str, Any] = {
            "model": self.model,
    ...
    async def embed(self, texts: list[str]) -> list[list[float]]:
        raise NotImplementedError("OpenAI adapter does not expose embeddings in this build")
